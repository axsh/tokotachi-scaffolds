...
5. **テスト項目 (Testing for the Requirements)**:
    ...
    * **ビルド・全体検証の書き方**:
        * **ビルド**: `scripts/process/build.sh` を記載する（全体ビルド＋単体テスト）。
        * **統合テスト**: `scripts/process/integration_test.sh` を記載するが、**オプションなしでの実行を推奨しない**。以下のガイドラインに従うこと。
    * **`integration_test.sh` の実行戦略**:
        * オプションなしで実行すると全カテゴリの統合テストが長時間実行され、不安定なテストも含まれるため、開発中の検証には不向き。全カテゴリ一括実行は Nightly build 等の用途に留める。
        * 仕様書では、**影響範囲を分析した上で、`--categories` や `--specify` オプションを使った具体的な実行コマンドを複数記述する**こと。
        * 利用可能なカテゴリ: `common`, `llm`, `taskengine`, `template`, `gui`
        * `--specify "REGEX"` で特定テストに絞り込むことも可能。
        * 記述例:
            ```
            ### ビルド・全体検証
            ...
            ```
        * 仕様の影響範囲に応じて、実行するカテゴリと `--specify` の対象を適切に選定する。影響しないカテゴリは含めない。
