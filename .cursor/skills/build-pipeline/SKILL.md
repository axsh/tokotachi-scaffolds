...
> **Linux / Remote-SSH（headless）**: ワークスペースが **Linux**、または **Remote-SSH の先が Linux** のときは、**`--headed` と `--ui` を付けない**（E2E は headless 既定）。`integration_test.sh` は **`xvfb-run -a` で必ずラップ**する（例: **`xvfb-run -a ./scripts/process/integration_test.sh`**。オプション付き再実行も同様。直接 `./scripts/process/integration_test.sh` を実行しない）。`xvfb-run` が無い場合は Xvfb 系パッケージを導入してから実行する（`features/frontend/scripts/integration_test.sh` 先頭コメント参照）。
...
> **Linux / Remote-SSH** では、このブロックおよび 5.2 の例にある **`./scripts/process/integration_test.sh` を必ず `xvfb-run -a` でラップ**する（例: `xvfb-run -a ./scripts/process/integration_test.sh --resume`）。
...
全てのビルドとテストが成功した時点で、`git push` を実施してリモートリポジトリに反映する。
